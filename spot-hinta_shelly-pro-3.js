...
let SETTINGS_PRICELIMIT_1 =
{
    ...
    PriceAllowed: "2", // Price limit (in euro cents, without decimals). If price is now below less than this the relay is turned ON (or OFF if inverted - see below)
    ...
    RelayName: "LVV1",  // Name this relay. Name is used in debug log mostly.
...
// Settings - Price limit - rule 2
let SETTINGS_PRICELIMIT_3 =
{
    // User settings
    RelayIsInUse: false, // True/false: If you want to use this relay or not
    PriceAllowed: "2", // Price limit (in euro cents, without decimals). If price is now below less than this the relay is turned ON (or OFF if inverted - see below)
    AllowedDays: "1,2,3,4,5,6,7", // Execution days: 1=Monday to 7=Sunday, separated with comma. 
    Relay: "2",  // Shelly's relay number (Value is between 0-3 depending how many relays Shelly has). Make sure this is correct!
    RelayName: "LVV3",  // Name this relay. Name is used in debug log mostly.
    Inverted: false, // True/false: Set to "true" to inverted the relay logic
...
let SETTINGS_RANK_PRICE_1 =
{
    ...
    PriceAllowed: "0",
    MaxPrice: "8", // Maximum allowed price in euro cents.
    AllowedDays: "1,2,3,4,5,6,7",
...
function ExecuteRelayRules() {
    ...
    ExecuteRelayRule(SETTINGS_PRICELIMIT_3);
    ExecuteRelayRule(SETTINGS_RANK_PRICE_1);
    ExecuteRelayRule(SETTINGS_RANK_PRICE_2);
    ExecuteRelayRule(SETTINGS_RANK_PRICE_3);
}
...
function ProcessHttpRequestResponse(response, error_code, error_msg, Settings) {
    ...
    if (Settings.SettingsNumber === 3) { SETTINGS_PRICELIMIT_3.RelayExecuted = response; }
    if (Settings.SettingsNumber === 4) { SETTINGS_RANK_PRICE_1.RelayExecuted = response; }
    if (Settings.SettingsNumber === 5) { SETTINGS_RANK_PRICE_2.RelayExecuted = response; }
    if (Settings.SettingsNumber === 6) { SETTINGS_RANK_PRICE_3.RelayExecuted = response; }
}
...
function SetRelayStatusInSettings(Settings, newStatus, relayStatusSource) {
    ...
    if (Settings.SettingsNumber === 3) { SETTINGS_PRICELIMIT_3.RelayStatus = newStatus; SETTINGS_PRICELIMIT_3.RelayStatusSource = relayStatusSource; }
    if (Settings.SettingsNumber === 4) { SETTINGS_RANK_PRICE_1.RelayStatus = newStatus; SETTINGS_RANK_PRICE_1.RelayStatusSource = relayStatusSource; }
    if (Settings.SettingsNumber === 5) { SETTINGS_RANK_PRICE_2.RelayStatus = newStatus; SETTINGS_RANK_PRICE_2.RelayStatusSource = relayStatusSource; }
    if (Settings.SettingsNumber === 6) { SETTINGS_RANK_PRICE_3.RelayStatus = newStatus; SETTINGS_RANK_PRICE_3.RelayStatusSource = relayStatusSource; }
}
...
function InitializeRelaysIfHourHasChanged(rounds) {
    ...
    if (currentHour !== currentHourUpdated || rounds === 1) {
        ...
        if (SETTINGS_PRICELIMIT_3.RelayIsInUse === true) { SETTINGS_PRICELIMIT_3.RelayExecuted = false } else { SETTINGS_PRICELIMIT_3.RelayExecuted = true; };
        ...
        if (SETTINGS_RANK_PRICE_3.RelayIsInUse === true) { SETTINGS_RANK_PRICE_3.RelayExecuted = false } else { SETTINGS_RANK_PRICE_3.RelayExecuted = true; };
        }
}
...
function HasCurrentHourBeenDone() {
    ...
    if (SETTINGS_PRICELIMIT_1.RelayExecuted === true &&
        ...
        SETTINGS_PRICELIMIT_3.RelayExecuted === true &&
        SETTINGS_RANK_PRICE_1.RelayExecuted === true &&
        SETTINGS_RANK_PRICE_2.RelayExecuted === true &&
        SETTINGS_RANK_PRICE_3.RelayExecuted === true) {
